---
title: "Benchmark: GPfit Learner Performance"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Benchmark: GPfit Learner Performance}
...
Ce document présente un benchmark complet du learner **GPfit** (Gaussian Process) 
pour la régression, comparé à trois autres algorithmes :

- **Featureless** : Baseline (prédiction de la moyenne)
- **CV-Glmnet** : Modèle linéaire avec régularisation élastique
- **KNN** : k plus proches voisins avec tuning automatique (k=1 à 30)
...
Nous utilisons deux tâches de régression basées sur le dataset **iris** :
```{r data}
...
## Visualisation
```{r plot, fig.cap="Erreur de prédiction en validation croisée à 5 folds"}
...
### Erreurs moyennes par algorithme
```{r mean_errors}
cat("\n=== Analyse des résultats ===\n")
...
## Question 1 : GPfit apprend-il quelque chose de non-trivial ?
```{r question1}
...
**Réponse** : **OUI**, GPfit apprend des patterns non-triviaux sur les deux datasets, 
avec des améliorations significatives par rapport au baseline featureless.
...
**Réponse** : **OUI**, GPfit est le **meilleur** algorithme sur les deux datasets, 
surpassant à la fois CV-Glmnet (modèle linéaire) et KNN (avec tuning optimal).
...
**GPfit (Gaussian Process)** démontre d'excellentes performances sur les deux 
datasets de régression :

1. **Apprentissage non-trivial** : Amélioration significative par rapport au baseline featureless
2. **Performance supérieure** : Meilleur que CV-Glmnet (modèle linéaire) et KNN (avec tuning)
3. **Stabilité** : Faible variance à travers les folds de validation croisée

Ces résultats confirment l'efficacité des Processus Gaussiens pour la régression 
sur des données de taille modérée avec des relations non-linéaires complexes.
